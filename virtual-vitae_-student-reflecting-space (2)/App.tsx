...
import { UserDetails, Vent, AppView } from './types.ts';
import FloatingParticles from './components/FloatingParticles.tsx';
import { getEmpatheticResponse } from './services/geminiService.ts';
...
const App: React.FC = () => {
  ...
  const handleFullReset = () => {
    if (confirm('This will permanently delete all your reflections and profile data. This cannot be undone. Are you sure?')) {
      if (user.email) {
  ...
  const handleSwitchProfile = () => {
    if (confirm('Return to the entry screen? Your current session reflections are saved locally.')) {
      setView('onboarding');
      setVentText('');
      setCurrentResponse(null);
      setHistory([]);
    ...
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 animate-fadeIn">
      <div className="glass p-8 rounded-3xl shadow-2xl max-w-md w-full text-center border border-white/40">
        ...
        <button 
          ...
          className="mt-8 text-[10px] text-gray-400 hover:text-red-400 transition-colors uppercase tracking-widest font-bold border-t border-gray-100 pt-6 w-full"
        >
          Reset Entire Space & Wipe All Data
        </button>
      </div>
    </div>
  );
  ...
  const renderVenting = () => (
    <div className={`flex flex-col items-center justify-start min-h-[80vh] px-4 pt-10 animate-fadeIn relative transition-opacity duration-500 ${showResetAnimation ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
